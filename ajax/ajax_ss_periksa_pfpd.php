...
if ($jalur == "K" OR $jalur == "H") {
    $sql = "SELECT id, jum_dokumen, jum_konf, unit_short, pib_nomor, pib_tanggal, pib_tanggal_x, jalur, upload_timestamp, importir, shipper, status FROM (SELECT src.pfpd_id id, src.jum_dokumen, dst.jum_konf, src.unit_short, src.pib_nomor, src.pib_tanggal, src.pib_tanggal_x, src.jalur, src.upload_timestamp, src.importir, src.shipper, src.status FROM (SELECT COUNT(a.id) jum_dokumen, a.pfpd_id, a.pib_nomor, DATE_FORMAT(a.pib_tanggal,'%d-%m-%Y') pib_tanggal_x, a.pib_tanggal pib_tanggal, a.jalur, a.upload_timestamp, a.importir, a.shipper, h.unit_short, CASE WHEN a.status = 1 THEN 'BELUM DIKIRIM' WHEN a.status = 2 THEN 'DIKIRIM' WHEN a.status = 3 THEN 'PEMERIKSAAN' WHEN a.status = 4 OR a.status = 5 THEN 'DITERIMA' WHEN a.status = 40 THEN 'REJECT' END AS `status` FROM dokap a INNER JOIN users g ON a.uploader_id = g.id INNER JOIN unit h ON g.unit_id = h.id WHERE a.pfpd_id = '$user_id' GROUP BY a.pib_nomor) src LEFT JOIN (SELECT COUNT(a.id) jum_konf, a.pib_nomor, a.pib_tanggal FROM konfirmasi a GROUP BY a.pib_nomor) dst ON src.pib_nomor = dst.pib_nomor AND src.pib_tanggal = dst.pib_tanggal) AS t WHERE t.pib_tanggal BETWEEN STR_TO_DATE('$minDate', '%d-%m-%Y') AND STR_TO_DATE('$maxDate', '%d-%m-%Y') AND t.jalur = '$jalur' ";
} else {
...
while( $row = mysqli_fetch_array($query) ) { 
    ...
    if ($jalur == "K") {
        $nestedData[] = '<span class="y-jalur">' . $row["pib_nomor"] . '</span.';
    } elseif ($jalur == "H") {
        $nestedData[] = '<span class="g-jalur">' . $row["pib_nomor"] . '</span.';
    } else {
        $nestedData[] = '<span">' . $row["pib_nomor"] . '</span.';
    }
    ...
    // $nestedData[] = $row["jum_konf"];
    $nestedData[] = '<div class="selesai_byid_detil_link" pib_nomor="'.$row['pib_nomor'].'" pib_tanggal="'.$row['pib_tanggal'].'">DETIL (KONF)</div>';
    $data[] = $nestedData;
}
